...
export default function LobbyPage() {
  ...
  function createRoom() {
    setMyRoom(uuidv4().substring(0, 8).toUpperCase());
  }
  function enterRoom() {
    if (!myRoom) return;
    socketRef.current?.emit("lobby_leave");
    ...
    const pw =
      createPasswordEnabled && createPassword
  ...
  function joinRoom() {
    ...
    router.push(
      `/game?mode=${modeParam}&room=${joinId.trim().toUpperCase()}${pw}`,
    );
  }
  ...
  function joinFromList(room: RoomItem, pw?: string) {
    ...
    if (room.gameMode === "egg") {
      router.push(`/lempar-telur?room=${room.id}`);
      return;
    }
    const modeParam = room.gameMode === "dino" ? "multi-dino" : "multi";
    const pwParam = pw ? `&pw=${encodeURIComponent(pw)}` : "";
    router.push(
      `/game?mode=${modeParam}&room=${room.id}&speed=${room.speed}${pwParam}`,
    );
  }

  function createBattleRoom() {
    setBattleRoom(uuidv4().substring(0, 8).toUpperCase());
  }
  ...
  function createEggRoom() {
    setEggRoom(uuidv4().substring(0, 8).toUpperCase());
  }
  ...
  const multiRooms = roomList.filter(
    (r) => !r.started && r.gameMode !== "battle" && r.gameMode !== "egg",
  );
  const battleRooms = roomList.filter(
    (r) => r.gameMode === "battle" && !r.started,
  );
    ...
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-[#9b7634] via-[#C4955A] to-[#8B5E3C] p-3 pb-6">
        ...
        <div className="fixed inset-0 z-100 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-7 flex flex-col items-center gap-3 max-w-xs w-full text-center">
            ...
            <p className="text-gray-600 text-sm">
              Akunmu login dari perangkat lain. Kamu akan diarahkan ke halaman
              login...
            </p>
            <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="h-full bg-pink-500 animate-[shrink_3s_linear_forwards] rounded-full"
                style={{ width: "100%" }}
              />
            </div>
          </div>
      ...
      {invite && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-white rounded-2xl shadow-2xl p-5 flex flex-col items-center gap-3 w-[90vw] max-w-sm border-2 border-pink-400">
          ...
          <p className="text-xs text-gray-500 font-mono">
            ...
            {invite.speed && (
              <span className="ml-2 text-yellow-600">⚡ {invite.speed}</span>
            )}
            <span className="ml-2">
      ...
      <div className="flex flex-col gap-3 w-full max-w-md">
        ...
        <div className="bg-white/20 backdrop-blur-md rounded-3xl shadow-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-extrabold text-base leading-none truncate">
                {user.username}
              </p>
        ...
        <div className="bg-white/20 backdrop-blur-md rounded-3xl shadow-2xl overflow-hidden">
          <div className="flex border-b border-white/15">
            {(["solo", "multi", "battle", "egg"] as ActiveTab[]).map((tab) => {
              ...
              const badges: Partial<Record<ActiveTab, number>> = {
                battle: battleRooms.length,
                multi: multiRooms.length,
                egg: eggRooms.length,
              };
              const badgeColors: Partial<Record<ActiveTab, string>> = {
                battle: "bg-red-500",
                multi: "bg-green-500",
                egg: "bg-yellow-400",
              };
              ...
              return (
                ...
                >
                  ...
                  {badge > 0 && (
                    <span
                      className={`absolute top-1 right-1 ${badgeColors[tab]} text-white text-[8px] font-bold w-3.5 h-3.5 rounded-full flex items-center justify-center`}
                    >
                      {badge}
                    </span>
                  )}
          ...
          {activeTab === "solo" && (
            <div className="p-4 space-y-3">
              ...
              <button
                onClick={playSolo}
                className="w-full py-3 bg-linear-to-r from-pink-500 to-rose-500 hover:from-pink-400 hover:to-rose-400 text-white text-base font-bold rounded-2xl shadow-lg transition active:scale-95"
              >
                🐷 Flappy Piggies Solo
              </button>
              <button
                onClick={playBaby}
                className="w-full py-3 bg-linear-to-r from-purple-500 to-indigo-500 hover:from-purple-400 hover:to-indigo-400 text-white text-base font-bold rounded-2xl shadow-lg transition active:scale-95"
              >
                👶 Baby Dino Solo
              </button>
              <button
                onClick={() => {
                  socketRef.current?.emit("lobby_leave");
                  router.push("/lempar-telur?room=solo");
                }}
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl font-bold text-sm transition active:scale-95 ${gameMode === "flappy" ? "bg-pink-500 text-white shadow" : "bg-white/10 text-white/60 hover:bg-white/20"}`}
                >
                  🐷 Flappy
                </button>
                <button
                  onClick={() => setGameMode("dino")}
                  className={`flex-1 py-2 rounded-xl font-bold text-sm transition active:scale-95 ${gameMode === "dino" ? "bg-purple-500 text-white shadow" : "bg-white/10 text-white/60 hover:bg-white/20"}`}
                >
                  👶 Dino
                </button>
              </div>
              ...
              {!myRoom ? (
                <button
                  onClick={createRoom}
                  className="w-full py-3 bg-linear-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white font-bold rounded-2xl shadow-lg transition active:scale-95"
                >
                ...
                <div className="bg-white/10 rounded-2xl p-3 border border-white/20">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-mono bg-white/20 text-yellow-200 font-extrabold px-3 py-1.5 rounded-lg flex-1 text-center text-lg tracking-widest">
                      {myRoom}
                    </span>
                    <button
                      onClick={() =>
                  ...
                  <label className="flex items-center gap-2 text-white/70 text-xs cursor-pointer mb-1">
                    <input
                      ...
                      onChange={(e) =>
                        setCreatePasswordEnabled(e.target.checked)
                      }
                  ...
                  {createPasswordEnabled && (
                    <input
                      ...
                      maxLength={20}
                      className="w-full px-3 py-1.5 rounded-xl bg-white/80 text-gray-700 text-sm outline-none focus:ring-2 focus:ring-green-300 mb-2"
                    />
                  )}
                  <button
                    onClick={enterRoom}
              ...
              <div className="bg-white/10 rounded-2xl p-3">
                ...
                <div className="flex gap-1.5 mb-2">
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg font-bold text-xs transition active:scale-95 ${joinGameMode === "flappy" ? "bg-pink-500 text-white" : "bg-white/10 text-white/50 hover:bg-white/20"}`}
                  >
                    🐷 Flappy
                  </button>
                  <button
                    onClick={() => setJoinGameMode("dino")}
                    className={`flex-1 py-1.5 rounded-lg font-bold text-xs transition active:scale-95 ${joinGameMode === "dino" ? "bg-purple-500 text-white" : "bg-white/10 text-white/50 hover:bg-white/20"}`}
                  >
                    👶 Dino
                  </button>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    onClick={() => socketRef.current?.emit("request_room_list")}
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-start gap-2 bg-white/10 rounded-2xl p-3">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs">
                    Turn-based 2–4 pemain: lempar bambu, pakai power-up,
                    habiskan HP lawan!
                  </p>
                </div>
                <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ml-auto">
                  NEW
                </span>
              </div>
              ...
              {!battleRoom ? (
                <button
                  ...
                  className="w-full py-3 bg-linear-to-r from-red-500 to-orange-500 hover:from-red-400 hover:to-orange-400 text-white font-bold rounded-2xl shadow-lg transition active:scale-95"
                >
                  ⚔️ Buat Battle Room
                </button>
              ) : (
              ...
              <div className="bg-white/10 rounded-2xl p-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={joinBattleId}
                    onChange={(e) =>
                      setJoinBattleId(e.target.value.toUpperCase())
                    }
                    maxLength={8}
                ...
                {battleRooms.length === 0 ? (
                  <div className="bg-white/5 rounded-2xl py-5 text-center">
                    ...
                    <p className="text-white/30 text-xs mt-1">
                      Buat room di atas atau tunggu pemain lain
                    </p>
                  </div>
                ) : (
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-start gap-2 bg-white/10 rounded-2xl p-3">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs">
                    Naiki platform setinggi mungkin! Maks 5 pemain, 1 nyawa —
                    siapa bertahan paling tinggi?
                  </p>
                </div>
                <span className="bg-yellow-400 text-gray-900 text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ml-auto">
                  NEW
                </span>
              </div>
              ...
              {!eggRoom ? (
                <button
                  onClick={createEggRoom}
                  className="w-full py-3 bg-linear-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-white font-bold rounded-2xl shadow-lg transition active:scale-95"
                >
                ...
                <div className="bg-white/10 rounded-2xl p-3 border border-white/20">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <button
                      onClick={() =>
                        navigator.clipboard?.writeText(eggRoom).catch(() => {})
                      }
                      className="px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-bold rounded-lg transition"
                    >
                      Salin
                  ...
                  <button
                    onClick={enterEggRoom}
                    className="w-full py-2.5 bg-yellow-500 hover:bg-yellow-400 text-white font-bold rounded-xl transition active:scale-95"
                  >
                    ▶️ Masuk ke Egg Room
                  </button>
                </div>
              ...
              <div className="bg-white/10 rounded-2xl p-3">
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Kode egg room..."
                    value={joinEggId}
                    onChange={(e) => setJoinEggId(e.target.value.toUpperCase())}
                    maxLength={8}
                    className="flex-1 px-3 py-2 rounded-xl bg-white/80 text-gray-700 font-mono font-bold text-base tracking-widest outline-none focus:ring-2 focus:ring-yellow-300"
                  />
                  <button
                    onClick={joinEggRoom}
                    className="px-4 py-2 bg-yellow-500 hover:bg-yellow-400 text-white font-bold rounded-xl transition active:scale-95"
                  >
                    Join
                  </button>
                </div>
                      ...
                      >
                        <div className="flex flex-col min-w-0">
                          ...
                          <p className="text-white/60 text-xs mt-0.5">
                            ...
                            · {room.playerCount}/5 pemain
                          </p>
                        </div>
                        <button
                          onClick={() => joinFromList(room)}
                          className="ml-2 shrink-0 px-3 py-1.5 bg-yellow-500 hover:bg-yellow-400 text-gray-900 text-xs font-bold rounded-lg transition active:scale-95"
                        >
                          Join 🥚
                        </button>
                      </div>
              ...
              {onlinePlayers.map((p) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span
                      className="inline-block w-6 h-6 rounded-full border-2 border-white/60 text-center text-xs leading-5"
                      style={{
        ...
        <div className="flex justify-center gap-4 text-xs text-white/40 pb-1">
          ...
          <a href="/account" className="hover:text-white underline">
            ⚙️ Akun
          </a>
        </div>
      </div>
