...
export default function BattleGame({
  ...
  useEffect(() => {
    sendResultRef.current = (hits) => {
      ...
      socketRef.current?.send("battle_throw_result", { hits });
    };
  });
  ...
  function mobileReleaseThrow() {
    ...
    socketRef.current?.send("battle_throw", {
      angle: gs.aimAngle,
      power,
      powerUp: pu,
      startX: me.x,
  ...
  const render = useCallback(() => {
    ...
    if (gs.currentTurnId === gs.myId && !gs.projectile) {
      ...
      if (me && me.alive) {
        ...
        if (moved && Date.now() - gs.moveLastEmit > 60) {
          ...
          socketRef.current?.send("battle_move", { x: me.x });
        }
      }
    }
    ...
    gs.players.forEach((p) => {
      ...
      if (p.character === "cat")
        drawCat(ctx, p.x, GROUND_Y, bodyColor, fr, !p.alive);
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((room) => {
        ...
        gsRef.current.myId = room.sessionId;
        setUiMyId(room.sessionId);
        room.onMessage("battle_room_state", (data) => {
          const gs = gsRef.current;
        ...
        room.onMessage("battle_game_start", (data) => {
          const gs = gsRef.current;
          gs.players = data.players;
        ...
        room.onMessage(
          ...
          (d: {
            ...
            const gs = gsRef.current;
            d.players.forEach((sp: BattlePlayer) => {
              const lp = gs.players.find((p) => p.id === sp.id);
              if (lp) Object.assign(lp, sp);
            });
            d.hits?.forEach((h: { targetId: string; damage: number }) => {
        ...
        room.onMessage("battle_game_over", (d) => {
          const gs = gsRef.current;
          gs.phase = "gameover";
        ...
        room.onMessage("battle_rematch_votes", (d) => setRematchVotes(d));
        room.onMessage("battle_player_left", (d) => {
          const gs = gsRef.current;
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-2 gap-3">
      ...
      {uiPhase === "waiting" && (
        <div className="bg-white/10 backdrop-blur-md rounded-3xl p-6 w-full max-w-md text-white shadow-2xl">
              ...
              return (
                ...
                >
                  ...
                  {!isMine && !occupant && (
                    <button
                      onClick={() =>
                        socketRef.current?.send("battle_pick_slot", {
                          slot: slotIdx,
                        })
                      }
          ...
          {isHost ? (
            <button
              onClick={() => socketRef.current?.send("battle_start")}
      ...
      {uiPhase === "gameover" && (
        <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 w-full max-w-md text-white shadow-2xl text-center">
          ...
          <div className="flex flex-col gap-2 mb-5">
            {[...uiPlayers]
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className={`flex items-center gap-3 rounded-xl px-4 py-2 ${p.id === gsRef.current.winnerId ? "bg-yellow-400/30 ring-1 ring-yellow-400" : "bg-white/10"}`}
                >
          ...
          {!voted ? (
            <button
              onClick={() => {
                socketRef.current?.send("battle_vote_rematch");
                setVoted(true);
              }}
