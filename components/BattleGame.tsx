...
interface PowerUps { big: boolean; double: boolean; explosive: boolean; }
...
const PIG_COLORS: Record<string, string> = {
  pink: "#ffc8d8", blue: "#a8d4ff", purple: "#d0a8ff",
  orange: "#ffd0a0", green: "#a8f0c0", yellow: "#fff0a0",
  red: "#ffb0a8", teal: "#a0e8e0", white: "#f0f0f0", brown: "#d4b090",
};
...
export default function BattleGame({ roomId, username, pigColor, character }: Props) {
  ...
  const [mobileSelectedPU, setMobileSelectedPU] = useState<PowerUpKey | null>(null);
  const [uiAimAngle, setUiAimAngle] = useState(45);

  // ── Character drawing ────────────────────────────────────────────────────

  function drawCat(ctx: CanvasRenderingContext2D, x: number, base: number, color: string, fr: boolean, dead: boolean) {
    ...
    ctx.bezierCurveTo(tx + (fr ? -24 : 24), base - 12, tx + (fr ? -32 : 32), base - 44, tx + (fr ? -22 : 22), base - 58);
    ctx.stroke(); ctx.lineCap = "butt";

    // body
    ctx.fillStyle = color; rr(ctx, x - 15, base - 40, 30, 32, 7); ctx.fill();

    // head
    ctx.fillStyle = color; ctx.beginPath(); ctx.arc(x, base - 52, 15, 0, Math.PI * 2); ctx.fill();

    // ears
    ctx.fillStyle = dk;
    ([[x - 10, base - 62, x - 17, base - 77, x - 3, base - 62],
      [x + 3, base - 62, x + 17, base - 77, x + 10, base - 62]] as number[][]).forEach(([ax, ay, bx, by, cx, cy]) => {
    ...
    ([[x - 10, base - 63, x - 14, base - 72, x - 4, base - 63],
      [x + 4, base - 63, x + 14, base - 72, x + 10, base - 63]] as number[][]).forEach(([ax, ay, bx, by, cx, cy]) => {
      ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(bx, by); ctx.lineTo(cx, cy); ctx.fill();
    });
    ...
    [-6, 6].forEach(ox => {
      ctx.beginPath();
      ctx.ellipse(x + eo + ox, base - 53, 2.5, dead ? 1 : 3.5, 0, 0, Math.PI * 2);
      ctx.fill();
    });
    if (!dead) {
      ctx.fillStyle = "#fff";
      [-6, 6].forEach(ox => { ctx.beginPath(); ctx.arc(x + eo + ox + 1, base - 55, 1, 0, Math.PI * 2); ctx.fill(); });
    }

    ctx.fillStyle = "#f88"; ctx.beginPath(); ctx.arc(x + eo * 0.5, base - 47, 2, 0, Math.PI * 2); ctx.fill();

    ctx.strokeStyle = dk; ctx.lineWidth = 1;
    [[-10, -1], [-10, -3], [10, -1], [10, -3]].forEach(([dx, dy]) => {
      ctx.beginPath(); ctx.moveTo(x + eo * 0.5, base - 47); ctx.lineTo(x + eo * 0.5 + dx, base - 47 + dy); ctx.stroke();
    });

    ctx.fillStyle = color;
    ctx.fillRect(x - 11, base - 10, 9, 10); ctx.fillRect(x + 2, base - 10, 9, 10);
    ctx.fillStyle = dk;
    ctx.beginPath(); ctx.ellipse(x - 6, base, 7, 4, 0, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(x + 6, base, 7, 4, 0, 0, Math.PI * 2); ctx.fill();

    ctx.globalAlpha = 1;
  }

  function drawDog(ctx: CanvasRenderingContext2D, x: number, base: number, color: string, fr: boolean, dead: boolean) {
    ctx.globalAlpha = dead ? 0.38 : 1;
    ...
    ctx.quadraticCurveTo(tx + (fr ? -28 : 28), base - 55, tx + (fr ? -18 : 18), base - 68);
    ctx.stroke(); ctx.lineCap = "butt";

    // body
    ctx.fillStyle = color; rr(ctx, x - 17, base - 42, 34, 34, 9); ctx.fill();

    // head
    ctx.fillStyle = color; rr(ctx, x - 16, base - 76, 32, 30, 9); ctx.fill();
    // snout
    ctx.fillStyle = lt; rr(ctx, x - 9, base - 56, 18, 14, 6); ctx.fill();
    // ears
    ctx.fillStyle = dk;
    rr(ctx, x - 24, base - 76, 11, 26, 5); ctx.fill();
    rr(ctx, x + 13, base - 76, 11, 26, 5); ctx.fill();

    const eo = fr ? 3 : -3;
    ctx.fillStyle = dead ? "#555" : "#111";
    [-7, 7].forEach(ox => { ctx.beginPath(); ctx.arc(x + eo + ox, base - 65, dead ? 2 : 3, 0, Math.PI * 2); ctx.fill(); });
    if (!dead) {
      ctx.fillStyle = "#fff";
      [-7, 7].forEach(ox => { ctx.beginPath(); ctx.arc(x + eo + ox + 1, base - 66, 1, 0, Math.PI * 2); ctx.fill(); });
    }

    ctx.fillStyle = dead ? "#888" : "#111";
    ctx.beginPath(); ctx.ellipse(x + eo * 0.3, base - 50, 5, 4, 0, 0, Math.PI * 2); ctx.fill();

    ctx.strokeStyle = dk; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.arc(x + eo * 0.3, base - 48, 4, 0, Math.PI); ctx.stroke();

    ctx.fillStyle = color;
    ctx.fillRect(x - 13, base - 9, 10, 9); ctx.fillRect(x + 3, base - 9, 10, 9);
    ctx.fillStyle = dk;
    ctx.beginPath(); ctx.ellipse(x - 8, base, 9, 5, 0, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(x + 8, base, 9, 5, 0, 0, Math.PI * 2); ctx.fill();
  ...
  function addSparks(x: number, y: number, n: number, color: string) {
    for (let i = 0; i < n; i++) {
      ...
      gsRef.current.particles.push({
        ...
        life: 18 + Math.floor(Math.random() * 15), maxLife: 33,
      });
    }
  }
  ...
  function addDmgText(x: number, y: number, dmg: number) {
    gsRef.current.texts.push({
      x, y, vy: -2.2, text: `-${dmg}`, color: "#FF1744",
      life: 65, maxLife: 65,
    });
  }

  function playSound(type: "throw" | "hit" | "explosion" | "ground" | "turn" | "win" | "select") {
  ...
  const sendResultRef = useRef<(hits: { targetId: string; damage: number }[]) => void>(() => {});
  ...
  function mobileStartCharge(e: React.PointerEvent) {
    ...
    const gs = gsRef.current;
    if (gs.phase !== "playing" || gs.currentTurnId !== gs.myId || gs.projectile || gs.isCharging) return;
    const me = gs.players.find(p => p.id === gs.myId);
  ...
  function mobileReleaseThrow() {
    ...
    gs.isCharging = false;
    if (gs.phase !== "playing" || gs.currentTurnId !== gs.myId || gs.projectile) return;
    const me = gs.players.find(p => p.id === gs.myId);
    if (!me || !me.alive) return;
    ...
    socketRef.current?.emit("battle_throw", { angle: gs.aimAngle, power, powerUp: pu, startX: me.x });
  }

  function mobileCancelCharge() {
    const gs = gsRef.current;
    if (gs.isCharging) { gs.isCharging = false; gs.chargePower = 0; }
  }
  ...
  const render = useCallback(() => {
    ...
    if (gs.phase !== "playing") { rafRef.current = requestAnimationFrame(render); return; }

    // ── Sky + clouds ────────────────────────────────────────────────────────
    const skyGrad = ctx.createLinearGradient(0, 0, 0, GROUND_Y);
    skyGrad.addColorStop(0, "#5bc8f5"); skyGrad.addColorStop(1, "#d4f1fc");
    ...
    // ── Ground ──────────────────────────────────────────────────────────────
    ctx.fillStyle = "#4CAF50"; ctx.fillRect(0, GROUND_Y, W, H - GROUND_Y);
    ctx.fillStyle = "#388E3C"; ctx.fillRect(0, GROUND_Y, W, 12);
    ctx.strokeStyle = "#2E7D32"; ctx.lineWidth = 1;
    for (let gx = 15; gx < W; gx += 32) {
      ctx.beginPath(); ctx.moveTo(gx, GROUND_Y); ctx.lineTo(gx - 4, GROUND_Y - 8); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(gx, GROUND_Y); ctx.lineTo(gx + 4, GROUND_Y - 8); ctx.stroke();
    }

    // ── Territory wall (center divider, shortened) ───────────────────────────
    {
      const WX = WALL_CX; const WW = WALL_HW * 2; const WT = WALL_TOP_Y; const WH = GROUND_Y - WT;
      // Drop shadow
      ctx.fillStyle = "rgba(0,0,0,0.18)";
      ctx.fillRect(WX + WW / 2, WT, 5, WH);
      // Wall top cap (rounded feel)
      ctx.fillStyle = "#6D4C41";
      ctx.beginPath(); ctx.ellipse(WX, WT, WW / 2 + 3, 6, 0, Math.PI, 0); ctx.fill();
      // Wall body gradient
      const wGrad = ctx.createLinearGradient(WX - WW / 2, 0, WX + WW / 2, 0);
      wGrad.addColorStop(0, "#4E342E"); wGrad.addColorStop(0.3, "#8D6E63");
      wGrad.addColorStop(0.7, "#6D4C41"); wGrad.addColorStop(1, "#3E2723");
      ctx.fillStyle = wGrad;
      ...
      for (let wy = WT; wy < GROUND_Y; wy += 16) {
        ...
        ctx.beginPath(); ctx.moveTo(WX - WW / 2 + off, wy); ctx.lineTo(WX - WW / 2 + off, Math.min(wy + 16, GROUND_Y)); ctx.stroke();
      }
      // Edge highlight
    ...
    if (gs.currentTurnId === gs.myId && !gs.projectile) {
      ...
      if (me && me.alive) {
        let moved = false;
        const moveMinX = me.character === "dog" ? 415 : 50;
        const moveMaxX = me.character === "cat" ? 385 : W - 50;
        if (gs.moveKeys.left) { me.x = Math.max(moveMinX, me.x - 2.5); moved = true; }
        if (gs.moveKeys.right) { me.x = Math.min(moveMaxX, me.x + 2.5); moved = true; }
    ...
    gs.players.forEach(p => {
      ...
      const opponents = gs.players.filter(o => o.id !== p.id && o.alive);
      let fr = true;
      if (opponents.length > 0) {
        const nearest = opponents.reduce((a, b) => Math.abs(b.x - p.x) < Math.abs(a.x - p.x) ? b : a);
        fr = nearest.x >= p.x;
      }

      if (isMyTurn) { ctx.shadowColor = "#FFD700"; ctx.shadowBlur = 18; }
      if (p.character === "cat") drawCat(ctx, p.x, GROUND_Y, bodyColor, fr, !p.alive);
      else drawDog(ctx, p.x, GROUND_Y, bodyColor, fr, !p.alive);
      ctx.shadowBlur = 0;

      // HP bar
      const bw = 62; const bh = 9;
      const bx = p.x - bw / 2; const by = GROUND_Y - 108;
      ctx.fillStyle = "rgba(0,0,0,0.55)";
      rr(ctx, bx - 1, by - 1, bw + 2, bh + 2, 3); ctx.fill();
      const hpR = p.hp / p.maxHp;
      ctx.fillStyle = hpR > 0.5 ? "#4CAF50" : hpR > 0.25 ? "#FFC107" : "#F44336";
      if (hpR > 0) { rr(ctx, bx, by, bw * hpR, bh, 3); ctx.fill(); }
      ...
      // Power-up icons
      if (p.alive) {
        const pwIcons: [PowerUpKey, string][] = [["big", "🔥"], ["double", "✌️"], ["explosive", "💣"]];
        ...
        avail.forEach(([k, icon], i) => {
          ...
          if (isMyTurn && gs.selectedPU === k) {
            ctx.fillStyle = "#FFD700"; rr(ctx, ipx - 9, iconY - 9, 18, 18, 4); ctx.fill();
          }
          ctx.font = "12px sans-serif"; ctx.fillText(icon, ipx, iconY + 2);
        });
      }

      if (!p.alive) {
        ctx.font = "22px sans-serif"; ctx.fillText("💀", p.x, GROUND_Y - 50);
      }
    });
    ...
    if (gs.currentTurnId === gs.myId && me && me.alive && !gs.projectile) {
      const opponents = gs.players.filter(p => p.id !== gs.myId && p.alive);
      let dir = 1;
      if (opponents.length > 0) {
        const nearest = opponents.reduce((a, b) => Math.abs(b.x - me.x) < Math.abs(a.x - me.x) ? b : a);
        dir = nearest.x >= me.x ? 1 : -1;
      }
      const rad = (gs.aimAngle * Math.PI) / 180;
      const spd = (gs.isCharging ? gs.chargePower : 0.5) * 16;
      let px = me.x; let py = GROUND_Y - 30;
      let vx = Math.cos(rad) * spd * dir;
      let vy = -Math.sin(rad) * spd;
      // Curved trajectory — first 1/5 only (11 steps)
      ctx.setLineDash([5, 5]); ctx.strokeStyle = "rgba(255,240,80,0.92)"; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(px, py);
      for (let i = 0; i < 11; i++) {
        px += vx; py += vy; vy += GRAVITY;
        if (py >= GROUND_Y || px < 0 || px > W) break;
        if (px >= WALL_CX - WALL_HW && px <= WALL_CX + WALL_HW && py >= WALL_TOP_Y) break;
        ctx.lineTo(px, py);
      }
    ...
    // ── Power bar ─────────────────────────────────────────────────────────────
    if (gs.isCharging) {
      gs.chargePower = Math.min(1, (Date.now() - gs.chargeStart) / MAX_CHARGE_MS);
      const bx = W / 2 - 90; const by = H - 36; const bw = 180; const bh = 16;
      ctx.fillStyle = "rgba(0,0,0,0.55)"; rr(ctx, bx - 2, by - 2, bw + 4, bh + 4, 5); ctx.fill();
      const col = gs.chargePower < 0.5 ? "#4CAF50" : gs.chargePower < 0.8 ? "#FFC107" : "#F44336";
      ctx.fillStyle = col; rr(ctx, bx, by, bw * gs.chargePower, bh, 4); ctx.fill();
      ctx.fillStyle = "white"; ctx.font = "bold 11px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(`💪 ${Math.round(gs.chargePower * 100)}%`, W / 2, by + bh + 16);
    }
    ...
    if (gs.projectile && gs.projectile.active) {
      ...
      if (proj.y >= GROUND_Y - 5) {
        ...
        if (proj.type === "explosive") {
          ...
          addSparks(proj.x, GROUND_Y, 18, "#FF6B35");
          playSound("explosion");
          if (isMyProj) {
            const hits: { targetId: string; damage: number }[] = [];
            gs.players.filter(p => p.id !== proj.ownerId && p.alive).forEach(p => {
              const dist = Math.abs(p.x - proj.x);
              if (dist < EXPLOSION_R) {
                hits.push({ targetId: p.id, damage: Math.round(DMG_EXPL_DIRECT + DMG_EXPL_AREA_MAX * (1 - dist / EXPLOSION_R)) });
              }
            });
            sendResultRef.current(hits);
          }
      ...
      // Player hit
        ...
        let hit = false;
        const targets = gs.players.filter(p => p.id !== proj.ownerId && p.alive);
        for (const t of targets) {
          const hitR = proj.type === "big" ? 36 : 26;
          const dy = (GROUND_Y - 42) - proj.y;
          const dx = t.x - proj.x;
          if (Math.sqrt(dx * dx + dy * dy) < hitR) {
            proj.active = false; hit = true;
            if (proj.type === "explosive") {
              gs.explosions.push({ x: proj.x, y: proj.y, life: 45, maxLife: 45, maxR: EXPLOSION_R });
              addSparks(proj.x, proj.y, 18, "#FF6B35");
              playSound("explosion");
              if (isMyProj) {
                const hits: { targetId: string; damage: number }[] = [];
                gs.players.filter(p => p.id !== proj.ownerId && p.alive).forEach(p2 => {
                  const dist = Math.sqrt((p2.x - proj.x) ** 2 + ((GROUND_Y - 42) - proj.y) ** 2);
                  if (dist < EXPLOSION_R) hits.push({ targetId: p2.id, damage: Math.round(DMG_EXPL_DIRECT + DMG_EXPL_AREA_MAX * (1 - dist / EXPLOSION_R)) });
                });
                sendResultRef.current(hits);
              }
            } else {
              ...
              playSound("hit");
              if (isMyProj) {
                const bd = DMG_BASE_MIN + Math.random() * (DMG_BASE_MAX - DMG_BASE_MIN);
                const dmg = Math.round(proj.type === "big" ? bd * DMG_BIG_MULT : bd);
      ...
      if (gs.projectile?.active) {
        ...
        const len = p.size * 2.4; const th = p.size * 0.45;
        ...
        [1, 2].forEach(i => {
          ...
          ctx.beginPath(); ctx.moveTo(jx, -th / 2); ctx.lineTo(jx, th / 2); ctx.stroke();
        });
        if (p.type === "big") {
          ctx.fillStyle = "rgba(255,200,0,0.65)";
          ctx.beginPath(); ctx.arc(0, 0, p.size * 0.9, 0, Math.PI * 2); ctx.fill();
        }
        if (p.type === "explosive") {
          ctx.fillStyle = "rgba(255,80,0,0.6)";
          ctx.beginPath(); ctx.arc(0, 0, p.size * 1.1, 0, Math.PI * 2); ctx.fill();
        }
        ctx.restore();
      }
    }
    ...
    gs.explosions.forEach(e => {
      ...
      ctx.strokeStyle = "#FF6B35"; ctx.lineWidth = 5;
      ctx.beginPath(); ctx.arc(e.x, e.y, r, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = "rgba(255,200,0,0.3)";
      ctx.beginPath(); ctx.arc(e.x, e.y, r * 0.65, 0, Math.PI * 2); ctx.fill();
    });
    ...
    if (gs.currentTurnId) {
      ...
      const remaining = Math.max(0, Math.ceil((gs.turnEndTime - Date.now()) / 1000));
      const isMe = gs.currentTurnId === gs.myId;
      const label = gs.awaitingDouble && isMe ? "⚔️ Lempar LAGI!" : isMe ? "⚔️ Giliran KAMU!" : `🎮 Giliran ${curP?.username ?? "?"}`;

      // Box with glow border
      ctx.shadowColor = isMe ? "#FFD700" : "#4488ff";
      ctx.shadowBlur = 14;
      ctx.fillStyle = isMe ? "rgba(160,110,0,0.92)" : "rgba(15,15,50,0.90)";
      rr(ctx, W / 2 - 135, 6, 270, 54, 12); ctx.fill();
      ctx.shadowBlur = 0;
      ctx.strokeStyle = isMe ? "#FFD700" : "#6699ff"; ctx.lineWidth = 2;
      rr(ctx, W / 2 - 135, 6, 270, 54, 12); ctx.stroke();
      ...
      const timerColor = remaining <= 5 ? "#FF5555" : remaining <= 10 ? "#FFBB00" : (isMe ? "#fff8c0" : "rgba(200,220,255,0.9)");
      ctx.font = "bold 13px sans-serif";
    ...
    if (gs.currentTurnId === gs.myId && !gs.projectile) {
      ...
      rr(ctx, 8, H - 52, 340, 44, 8); ctx.fill();
      ctx.fillStyle = "rgba(255,255,255,0.82)";
      ctx.font = "10px sans-serif"; ctx.textAlign = "left";
      ctx.fillText("🎮  A / D : Gerak    ↑ ↓ : Sudut    [Space] : Tahan + Lepas = Lempar", 14, H - 35);
      ctx.fillText("💡  [1] 🔥 Besar    [2] ✌️ Dua Kali    [3] 💣 Ledak", 14, H - 20);

      ctx.fillStyle = "rgba(255,255,255,0.7)";
      ctx.font = "10px sans-serif"; ctx.textAlign = "right";
      ctx.fillText(`Sudut: ${gsRef.current.aimAngle}°`, W - 10, H - 20);
      if (gsRef.current.selectedPU) {
        const lbl: Record<PowerUpKey, string> = { big: "🔥 Besar aktif", double: "✌️ Dua Kali aktif", explosive: "💣 Ledak aktif" };
        ctx.fillStyle = "#FFD700"; ctx.fillText(lbl[gsRef.current.selectedPU], W - 10, H - 36);
      }
    }

    // Auto-skip when timer expires
    if (gs.currentTurnId === gs.myId && !gs.projectile && gs.turnEndTime > 0 && Date.now() > gs.turnEndTime) {
      gs.isCharging = false; gs.chargePower = 0; gs.turnEndTime = 0;
  ...
  useEffect(() => {
    function kd(e: KeyboardEvent) {
      ...
      switch (e.code) {
        case "ArrowLeft": case "KeyA": e.preventDefault(); gs.moveKeys.left = true; break;
        case "ArrowRight": case "KeyD": e.preventDefault(); gs.moveKeys.right = true; break;
        case "ArrowUp":
          e.preventDefault();
          if (!gs.projectile) { gs.aimAngle = Math.min(85, gs.aimAngle + 5); setUiAimAngle(gs.aimAngle); }
          break;
        case "ArrowDown":
          e.preventDefault();
          if (!gs.projectile) { gs.aimAngle = Math.max(5, gs.aimAngle - 5); setUiAimAngle(gs.aimAngle); }
          break;
        case "Space": case "KeyZ":
          e.preventDefault();
          if (!gs.isCharging && !gs.projectile) {
            gs.isCharging = true; gs.chargeStart = Date.now(); gs.chargePower = 0;
          }
          break;
        case "Digit1": if (me.powerUps.big) { const n = gs.selectedPU === "big" ? null : "big"; gs.selectedPU = n; if (n) playSound("select"); } break;
        case "Digit2": if (me.powerUps.double) { const n = gs.selectedPU === "double" ? null : "double"; gs.selectedPU = n; if (n) playSound("select"); } break;
        case "Digit3": if (me.powerUps.explosive) { const n = gs.selectedPU === "explosive" ? null : "explosive"; gs.selectedPU = n; if (n) playSound("select"); } break;
      }
    }
    ...
    function ku(e: KeyboardEvent) {
      ...
      if (e.code === "ArrowRight" || e.code === "KeyD") gs.moveKeys.right = false;
      ...
      if ((e.code === "Space" || e.code === "KeyZ") && gs.isCharging) {
        ...
        const power = Math.min(1, (Date.now() - gs.chargeStart) / MAX_CHARGE_MS);
        gs.chargePower = 0;
        const pu = gs.selectedPU; gs.selectedPU = null;
        if (pu) me.powerUps[pu] = false;
  ...
  useEffect(() => {
    ...
    const socket = socketIO(exUrl || (typeof window !== "undefined" ? window.location.origin : ""), {
      path,
    ...
    socket.on("battle_room_state", (data: { players: BattlePlayer[]; host: string; started: boolean }) => {
      const gs = gsRef.current;
    ...
    socket.on("battle_game_start", (data: { players: BattlePlayer[]; currentTurnId: string }) => {
      const gs = gsRef.current;
      gs.players = data.players;
      ...
      gs.projectile = null;
      gs.particles = []; gs.texts = []; gs.explosions = [];
      setUiPlayers([...data.players]);
      setUiPhase("playing");
      setVoted(false);
      setMobileSelectedPU(null);
    ...
    socket.on("battle_projectile", (d: {
      throwerId: string; angle: number; power: number;
      powerUp: PowerUpKey | null; startX: number;
    }) => {
      const gs = gsRef.current;
      const thrower = gs.players.find(p => p.id === d.throwerId);
      const opponents = gs.players.filter(p => p.id !== d.throwerId && p.alive);
      let dir = 1;
      if (thrower && opponents.length > 0) {
        const nearest = opponents.reduce((a, b) => Math.abs(b.x - thrower.x) < Math.abs(a.x - thrower.x) ? b : a);
        dir = nearest.x >= thrower.x ? 1 : -1;
      }
      const rad = (d.angle * Math.PI) / 180;
      const spd = d.power * 16;
      const type = d.powerUp === "big" ? "big" : d.powerUp === "explosive" ? "explosive" : "normal";
      gs.projectile = {
        x: d.startX, y: GROUND_Y - 30,
        vx: Math.cos(rad) * spd * dir, vy: -Math.sin(rad) * spd,
        size: type === "big" ? 16 : 10,
        type, ownerId: d.throwerId, active: true,
      };
      gs.awaitingDouble = false;
      playSound("throw");
    });

    socket.on("battle_state_update", (d: {
      players: BattlePlayer[]; currentTurnId: string;
      ...
      gs.turnEndTime = Date.now() + TURN_SECS * 1000;
      gs.projectile = null; gs.isCharging = false; gs.chargePower = 0;
      setUiPlayers([...gs.players]);
      setMobileSelectedPU(null);
      playSound("turn");
    });

    socket.on("battle_game_over", (d: { winnerId: string; winnerName: string; players: BattlePlayer[] }) => {
      const gs = gsRef.current;
      gs.phase = "gameover"; gs.winnerId = d.winnerId; gs.winnerName = d.winnerName;
      gs.players = d.players; gs.projectile = null;
      setUiPhase("gameover"); setUiWinner(d.winnerName); setUiPlayers([...d.players]);
      playSound("win");
    });

    socket.on("battle_rematch_votes", (d: { votes: number; total: number }) => setRematchVotes(d));
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-2 gap-3">
      ...
      <div className="flex items-center gap-3 w-full max-w-200">
        <a href="/lobby" className="text-white/60 hover:text-white text-sm underline shrink-0">← Lobby</a>
        <span className="text-white font-bold text-base flex-1 text-center">⚔️ Cat vs Dog Battle</span>
        <span className="bg-white/10 text-yellow-200 font-mono text-xs px-2 py-1 rounded-full shrink-0">{roomId}</span>
      </div>

      {/* Waiting room */}
      {uiPhase === "waiting" && (
        <div className="bg-white/10 backdrop-blur-md rounded-3xl p-6 w-full max-w-md text-white shadow-2xl">
          <h2 className="text-xl font-extrabold text-center mb-4">⚔️ Battle Room</h2>
          <div className="bg-white/10 rounded-2xl p-3 mb-4 text-center">
            <p className="text-white/60 text-xs mb-1">Kode Room</p>
            <p className="font-mono font-extrabold text-2xl tracking-widest text-yellow-200">{roomId}</p>
          </div>
          <p className="text-white/70 text-sm text-center mb-3">
            Pilih posisi ({uiPlayers.length}/4) — butuh minimal 2:
          </p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-5">
            {[0, 1, 2, 3].map((slotIdx) => {
              ...
              const charLabel = slotIdx % 2 === 0 ? "Cat" : "Dog";
              const sideLabel = slotIdx < 2 ? (slotIdx === 0 ? "Kiri-Jauh" : "Kanan-Jauh") : (slotIdx === 2 ? "Kiri-Dekat" : "Kanan-Dekat");
              return (
                <div
                  key={slotIdx}
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div>
                      <p className="text-white font-bold text-xs">{charLabel} — {sideLabel}</p>
                      {occupant ? (
                        <p className="text-white/80 text-xs font-semibold truncate max-w-[100px]">
                          {occupant.username}
                          {isMe && <span className="text-green-300 ml-1">(Kamu)</span>}
                          {occupant.id === uiHost && <span className="text-yellow-300 ml-1">👑</span>}
                        </p>
                      ) : (
                  ...
                  {!isMine && !occupant && (
                    <button
                      onClick={() => socketRef.current?.emit("battle_pick_slot", { slot: slotIdx })}
                  ...
                  {isMine && (
                    <p className="text-yellow-200 text-xs text-center font-bold">✓ Posisimu</p>
                  )}
                </div>
              );
            })}
          ...
          {uiPlayers.length < 2 && (
            <p className="text-white/40 text-xs text-center mb-3">Butuh minimal 2 pemain...</p>
          )}
          ...
          {isHost ? (
            ...
            <div className="text-center text-white/50 py-2 text-sm">Menunggu host memulai...</div>
          )}

          <div className="mt-4 bg-black/20 rounded-xl p-3 text-white/50 text-xs space-y-1">
            <p>🎮 <b>A / D</b> — Gerak kiri/kanan</p>
            <p>🎯 <b>↑ ↓</b> — Atur sudut lemparan</p>
            <p>💨 <b>[Space]</b> — Tahan untuk isi daya, lepas untuk lempar</p>
            <p>⚡ <b>[1] [2] [3]</b> — Pilih power-up sebelum lempar</p>
            <p>🔥 Big &nbsp;|&nbsp; ✌️ Dua Kali &nbsp;|&nbsp; 💣 Lemparan Ledak</p>
          </div>
        </div>
      )}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1.5 shrink-0">
              <button
                ...
                onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); gsRef.current.moveKeys.left = true; }}
                onPointerUp={() => { gsRef.current.moveKeys.left = false; }}
                onPointerCancel={() => { gsRef.current.moveKeys.left = false; }}
              >◀</button>
              <button
                className="w-16 h-16 rounded-2xl bg-white/25 active:bg-white/50 text-white text-2xl font-bold shadow-lg touch-none flex items-center justify-center"
                onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); gsRef.current.moveKeys.right = true; }}
                onPointerUp={() => { gsRef.current.moveKeys.right = false; }}
                onPointerCancel={() => { gsRef.current.moveKeys.right = false; }}
              >▶</button>
            </div>
            ...
            <div className="flex flex-col gap-1 shrink-0 items-center">
              <button
                ...
                onClick={() => {
                  const gs = gsRef.current;
                  if (gs.currentTurnId === gs.myId && !gs.projectile) { gs.aimAngle = Math.min(85, gs.aimAngle + 5); setUiAimAngle(gs.aimAngle); }
                }}
              >↑ Sudut</button>
              <span className="text-yellow-200 font-bold text-sm leading-none">{uiAimAngle}°</span>
              <button
                className="px-4 py-2 rounded-xl bg-white/20 active:bg-white/40 text-white text-sm font-bold shadow-lg w-full"
                onClick={() => {
                  const gs = gsRef.current;
                  if (gs.currentTurnId === gs.myId && !gs.projectile) { gs.aimAngle = Math.max(5, gs.aimAngle - 5); setUiAimAngle(gs.aimAngle); }
                }}
              >↓ Sudut</button>
            </div>
            ...
            >
              ...
              <span className="text-xs font-normal opacity-75">Tahan → Lepas</span>
            </button>
          </div>
      ...
      {uiPhase === "gameover" && (
        <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 w-full max-w-md text-white shadow-2xl text-center">
          ...
          <p className="text-white/50 text-sm mb-5">Cat vs Dog Battle selesai</p>
          <div className="flex flex-col gap-2 mb-5">
            {[...uiPlayers]
              .sort((a, b) => (b.alive ? 1 : 0) - (a.alive ? 1 : 0) || b.hp - a.hp)
              .map(p => (
                <div key={p.id} className={`flex items-center gap-3 rounded-xl px-4 py-2 ${p.id === gsRef.current.winnerId ? "bg-yellow-400/30 ring-1 ring-yellow-400" : "bg-white/10"}`}>
                  <span className="text-xl">{p.character === "cat" ? "🐱" : "🐶"}</span>
                  <span className="font-bold flex-1 text-left">{p.username}</span>
                  <span className={`text-sm font-bold ${p.alive ? "text-green-300" : "text-red-400"}`}>
                    {p.alive ? `${p.hp} HP` : "💀"}
                  </span>
                </div>
              ))}
          ...
          {!voted ? (
            <button
              onClick={() => { socketRef.current?.emit("battle_vote_rematch"); setVoted(true); }}
            ...
            <p className="text-green-300 font-bold mb-3">✓ Votemu tercatat — menunggu pemain lain...</p>
          )}
          <a href="/lobby" className="text-white/50 hover:text-white text-sm underline">← Kembali ke Lobby</a>
        </div>
      )}
    </div>
  );
